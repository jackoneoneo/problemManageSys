	...
	$scope.getData = function(page,setPwd) {
		...
		httpService.callHttpMethod($http,params).success(function(result) {
			...
			if(!isJson(result)||isSuccess.indexOf('[ERROR]') >= 0){
				...
				$rootScope.sessionAdminTrueName = result.data.sesssionAdminTrueName;
				...
				$rootScope.sessionAuth = result.data.sessionAuth;
				$rootScope.sessionAdminId =result.data.sessionAdminId;
				$rootScope.errorTips = result.data.errorTips;
				var length = result.data.menuList.length;
				var array = new Array();
				var i =0;
			     console.log($rootScope.sessionAdminAuthor);
				for(var index = 0;index<length;index++){
					
					
					if($rootScope.sessionAdminAuthor.indexOf($rootScope.backMenus[index].menu.id)>=0){
						array[i] = $rootScope.backMenus[index];
						i++;
					}else{
						//TODO
					}
				}
				$rootScope.userMenus = array;
			}
		});
    };
    // 判断是否是最高管理员
