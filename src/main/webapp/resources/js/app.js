...
app.run(function($rootScope, $state, $stateParams, $http, $timeout,httpService) {
    ...
    //detail('新增文件',800,'back/source/detail.do?id=NULL','&directoryId='+$stateParams.directoryId)
	 ...
	 * 文件提交的表单
	 */
	$rootScope.fileSubmitForm = function(url){
		if(url.indexOf('?')>=0){
	     	var iurl = url.substr(0,url.indexOf('?'));
	     	var fileRank = url.substr(url.indexOf('?')+1);
	     	$rootScope.model.fileRank = fileRank;
	     	$rootScope.submitForm(iurl);
	     }else{
	    	 $rootScope.submitForm(url);
	     }
